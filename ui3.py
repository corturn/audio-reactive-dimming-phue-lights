...
# It never touches Tkinter, so macOS won't crash it.
def keyboard_listener_process(queue):
    # We only import pynput inside this isolated child process
    from pynput import keyboard

    def on_press(key):
        try:
            if hasattr(key, 'char') and key.char:
                queue.put(('press', key.char.lower()))
        except Exception:
            pass
    ...
    def on_release(key):
        try:
            if hasattr(key, 'char') and key.char:
                queue.put(('release', key.char.lower()))
        except Exception:
            pass
...
class App(tk.Tk):
    ...
    def init_main_app(self):
        ...
        # ====================================================
        # SPAWN THE INVISIBLE CHILD PROCESS FOR THE KEYBOARD
        # ====================================================
        self.key_queue = multiprocessing.Queue()
        self.key_process = multiprocessing.Process(
            target=keyboard_listener_process, 
            args=(self.key_queue,), 
            daemon=True
        )
        ...
        self.poll_keyboard_queue() # Start checking the child process
        self.poll_audio()
        self.update_meter()
        self.update_lights()
    ...
    def poll_keyboard_queue(self):
        """Reads keystrokes piped over from the isolated child process."""
        while not self.key_queue.empty():
            try:
                event_type, char = self.key_queue.get_nowait()
        ...
        # Check the queue extremely fast (every 10ms) to ensure zero input lag
        self.after(10, self.poll_keyboard_queue)
