...
from passthrough3 import AudioSensor, AudioDevice
...
class App(tk.Tk):
    def __init__(self, sensor: AudioSensor):
        ...
        # --- NEW: Transient Hook ---
        self.sensor.on_transient_callback = self.flash_light
    ...
    def flash_light(self):
        ...
        # 0.1 second cooldown to prevent crashing the Bridge on fast drum rolls
        if current_time - self.last_transient_time > 0.1:
            try:
                # 0 transition time for an absolute instant flash to max brightness
                command = {'transitiontime': 0, 'bri': 254, 'on': True}
                threading.Thread(
                ...
                self.last_brightness = 254
                self.is_light_on = True
                self.last_transient_time = current_time
            except Exception as e:
                print(f"Transient Hue Error: {e}")
